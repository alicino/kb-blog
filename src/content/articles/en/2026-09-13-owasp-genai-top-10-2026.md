...
flowchart LR
    subgraph ATTACK[Attack]
        A1[Malicious input<br>text, image, audio] --> B1[Prompt injection]
        B1 --> C1[Unauthorized tool]
        C1 --> D1[Exfiltration or action]
    end

    subgraph DEFENSE[Layered defense]
        A2[Input guardrails] --> B2[Output validation]
        B2 --> C2[Human-in-the-loop<br>critical actions]
        C2 --> D2[DLP and monitoring]
    end
```
