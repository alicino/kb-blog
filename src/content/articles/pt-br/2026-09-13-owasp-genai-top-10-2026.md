...
flowchart LR
    subgraph ATTACK[Ataque]
        A1[Input malicioso<br>texto, imagem, áudio] --> B1[Prompt injection]
        B1 --> C1[Ferramenta não autorizada]
        C1 --> D1[Exfiltração ou ação]
    end

    subgraph DEFENSE[Defesa em camadas]
        A2[Guardrails de input] --> B2[Validação de output]
        B2 --> C2[Human-in-the-loop<br>ações críticas]
        C2 --> D2[DLP e monitoria]
    end
```
