...
```mermaid
flowchart LR
    subgraph ATTACK[Cadeia típica de ataque]
        A[Input malicioso<br>texto, imagem, áudio] --> B[Prompt injection]
        B --> C[Agente executa<br>ferramenta não autorizada]
        C --> D[Dados extraídos<br>ou ação executada]
    end

    subgraph DEFENSE[Defesa em camadas]
        E[Guardrails<br>de input] --> F[Validação<br>de output]
        F --> G[Human-in-the-loop<br>ações críticas]
        G --> H[DLP e monitoria]
    end
```
